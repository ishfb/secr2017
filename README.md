...
* Перечисляем проблемы:
  * Роутинг и логика обработки запроса идут вперемежку
  * Нет гарантии, что сервер возвращает ответ
  * Ответы хардкодятся каждый раз при выводе в поток - высокая вероятность ошибиться.
* Говорим, что будем бороться с последней проблемой - напишем класс `HttpResponse`, который будет инкапсулировать в себе строковое представление HTTP-ответов и предоставлять удобный интерфейс для их создания
* Поехали:
  1. Первый шаг - создали интерфейс
     * важно, чтобы программа компилировалась
     * поэтому мы реализуем интерфейс с помощью пустышек
  ...
  9. Ремарка про то, что мы не проверили, что вся система целиком продолжила работать правильно - для этого надо иметь функциональные тесты
...
* Эти шаги не применяются изолированно и бездумно:
   * все равно заранее надо продумать, как будет выглядеть конечный результат
   * другими словами, разрабатывая интерфейс, надо хотя бы прикинуть, как мы будем его реализовывать и тестировать
* Переходя от шага к шагу, мы все равно держим в голове решаемую задачу, просто на каждом шаге мы концентрируемся на чём-то одном
* Шаги я взял не с потолка, они во многом основаны на принципах TDD и книге Мартина Фаулера "Рефакторинг"
